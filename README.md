...
- **Model, effort and fast mode** sit in the same bar and change the current session. Defaults for new sessions live in Settings (**⌘,**). The list is what your Claude Code and Codex offer today, so a new model shows up without an update; each model has its own efforts, and fast mode appears only where the model has it (Opus; most GPT models) and costs more of your plan. Picking a model on the other agent keeps the conversation: what was said so far is handed over, a thin line marks where it moved, and the sidebar still shows one conversation.
